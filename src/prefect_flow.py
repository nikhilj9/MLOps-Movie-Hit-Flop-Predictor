...
def training_flow():
    ...
    df = load_and_preprocess_data_task(config["data"]["path"], config["model"]["genres"], config["model"]["target"])
    X_train, X_test, y_train, y_test = split_data_task(df, config["model"]["genres"], config["model"]["target"])
